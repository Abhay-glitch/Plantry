...
function Cpadmin() {
    ...
    const onhandleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.get("http://localhost:5000/user/fetch", { email, password: Opass });
    
            if (response.data) { // Assuming API returns { success: true } if password matches
                let updateDetail = {
                    condition_obj: { email },
